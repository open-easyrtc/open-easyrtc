...
eventListener.onConnection = function(socket, easyrtcid, next) {
    ...
    socket.on("easyrtcAuth", function(msg, socketCallback) {
        ...
        pub.events.emit("easyrtcAuth", socket, easyrtcid, msg, socketCallback, function(err, connectionObj) {
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
                ...
                async.each(Object.keys(msg.msgData.roomJoin), function(currentRoomName, roomCallback) {
                    ...
                    appObj.isRoom(currentRoomName, function(err, isRoom) {
                        ...
                        if (isRoom) {
                            ...
                            // Join existing room
                            appObj.room(currentRoomName, function (err, appRoomObj) {
                                ...
                                if (pub.getOption("roomMaxConnection")) {
                                    const connectionCount = appRoomObj.getConnectionCountSync()
                                    
                                    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] roomMaxConnection", {
                                        connectionCount: connectionCount, 
                                        roomMaxConnection: pub.getOption("roomMaxConnection")
                                    })

                                    if (connectionCount >= pub.getOption("roomMaxConnection")) {
                                        roomCallback(new pub.util.ConnectionError("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"]["+currentRoomName+"] Unable to join room, maximum connections reached."));
                                        return;
                                    }
                                }
            ...
            // If no room is initially provided, have them join the default room (if enabled)
                ...
                // Join default room
                var currentRoomName = connectionObj.getApp().getOption("roomDefaultName")
                appObj.room(currentRoomName, function (err, appRoomObj) {
                    ...
                    if (pub.getOption("roomMaxConnection")) {
                        ...
                        if (connectionCount >= pub.getOption("roomMaxConnection")) {
                            asyncCallback(new pub.util.ConnectionError("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"]["+currentRoomName+"] Unable to join room, maximum connections reached."));
                            return;
                        }
                    }

                    pub.events.emit("roomJoin", connectionObj, currentRoomName, null, function(err, roomObj) {
                        asyncCallback(err);
                    });
                })
            }
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
            ...
            if (msg.targetEasyrtcid !== undefined) {
                ...
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] EasyRTC message received without targetEasyrtcid or targetRoom or targetGroup", msg);
                next(null);
            }
        }
    ],
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            pub.socketServer.sockets.on("connection", function (socket) {
                if (socket.connected) { // Only if connection is still open
