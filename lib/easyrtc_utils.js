...
easyrtc_utils.sendSessionCookie = function(req, res) {
    ...
    const server = rep.app.get('easyrtcServer');
...
easyrtc_utils.isValidIncomingAuth = (type, msg, obj, callback) => {
    ...
    async.waterfall([
            function(asyncCallback) {
                pub.app((msg.msgData.applicationName !== undefined ? msg.msgData.applicationName : obj.getOption("appDefaultName")), function(err, newAppObj) {
                    if (!err) {
                        obj = newAppObj;
                    }
                    asyncCallback(null);
