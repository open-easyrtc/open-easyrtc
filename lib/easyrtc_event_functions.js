    ...
    socket.on("easyrtcAuth", function(msg, socketCallback) {
        ...
        async.waterfall([
            ...
            function(newConnectionObj, callback) {
                ...
                // TODO: Set socket.on("easyrtcCmd")
                socket.on("easyrtcCmd", function(msg, socketCallback){
                    pub.eventHandler.emit("easyrtcCmd", connectionObj, msg, socketCallback);
                });
                // TODO: Set socket.on("easyrtcMsg")
                socket.on("easyrtcMsg", function(msg, socketCallback){
                    pub.eventHandler.emit("easyrtcMsg", connectionObj, msg, socketCallback);
                });
                // TODO: Set socket.on("disconnect")
...
/*
        ...
        var localCb = function() {
            ...
            if(!_.isFunction(socketCallback)) {
                pub.util.logWarning("["+easyrtcid+"] easyRTC Auth message received with no callback", msg);
                socketCallback = function(returnMsg) {};
            }
    
            // If message has no msgType, then ignore it.
            if (!msg || !_.isString(msg.msgType)) {
                pub.util.logWarning("["+easyrtcid+"] easyRTC Auth message received with no msgType. Ignoring.", msg);
                return;
            }
    
            pub.util.logDebug("["+easyrtcid+"] easyRTC Auth message received of type [" + msg.msgType + "]", msg);

            pub.eventHandler.emit("easyrtcCmd", connectionObj, msg, socketCallback, function(err){
                if(err){eu.socketDisconnect(connectionObj.socket);pub.util.logError("["+easyrtcid+"] Unhandled easyrtcAuthhandler error. Disconnecting socket.", err);return;}
            });
        };
...
/*
    ...
    socket.on("easyrtcCmd", function(connectionObj, msg, socketCallback) {
...
module.exports.onEasyrtcMsg = function(connectionObj, msg, socketCallback, next){
    pub.util.logDebug("Running func 'onEasyrtcMsg'");
    if (!_.isFunction(next)) {
        next = function(err) {};
    }
    if (msg.targetEasyrtcid) {
...
module.exports.onEmitEasyrtcMsg = function(connectionObj, msgType, msg, socketCallback, next){
    ...
    pub.util.logDebug("["+msg.easyrtcid+"] is sent easyrtcMsg", msg);
...
module.exports.onEmitError      = function(connectionObj, errorCode, socketCallback, next){
    ...
    var msg = {msgData:{}};
    msg.msgData.errorCode = errorCode;
    msg.msgData.errorText = pub.util.getErrorText(errorCode);
    
    if (!msg.msgData.errorText) {
        msg.msgData.errorText="Error occured with error code: " + msg.errorCode;
    }

    pub.eventHandler.emit("emitEasyrtcCmd", connectionObj, "error", msg, socketCallback, next);
};
