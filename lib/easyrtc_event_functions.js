 ...
 * @module      easyrtc_event_functions
...
module.exports.onConnection     = function(socket, easyrtcid, next){
    ...
    socket.on("easyrtcAuth", function(msg, socketCallback) {
        pub.events._eventHandler.emit("easyrtcAuth", socket, easyrtcid, msg, socketCallback, function(err, newConnectionObj){
            if(err){
...
module.exports.onEasyrtcAuth    = function(socket, easyrtcid, msg, socketCallback, callback){
    ...
    async.waterfall([
        ...
        function(isMsgValid, msgErrorCode, asyncCallback) {
            ...
            pub.events._eventHandler.emit("authenticate", socket, easyrtcid, appName, username, credential, msg, asyncCallback);
        },
            ...
            if (msg.msgData.roomJoin) {
                for (var currentRoomName in msg.msgData.roomJoin) {
                    if (!_.isString(currentRoomName) || !appObj.getOption("roomNameRegExp").test(currentRoomName)) {
                        pub.events._eventHandler.emit("emitReturnError", socketCallback, "MSG_REJECT_TARGET_ROOM", function(err){});
        ...
        function(asyncCallback) {
            ...
            socket.on("easyrtcCmd", function(msg, socketCallback){
                pub.events._eventHandler.emit("easyrtcCmd", connectionObj, msg, socketCallback, function(err){
            ...
            socket.on("easyrtcMsg", function(msg, socketCallback){
                pub.events._eventHandler.emit("easyrtcMsg", connectionObj, msg, socketCallback, function(err){
                    if(err){pub.util.logError("["+easyrtcid+"] Unhandled easyrtcMsg handler error.", err);return;}
                });
            });
            socket.on("disconnect", function(){
                pub.events._eventHandler.emit("disconnect", connectionObj, function(err){
                ...
                async.each(Object.keys(msg.msgData.roomJoin), function(currentRoomName, roomCallback) {
                    ...
                    appObj.isRoom(currentRoomName, function(err, isRoom){
                        if (isRoom) {
                            ...
                            connectionObj.getApp().createRoom(currentRoomName, null, function(err, roomObj){
                                if (err) {
                                    roomCallback(err);
                                    return;
                                }
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
            ...
            switch(msg.msgType) {
                ...
                case "setPresence":
                    pub.events._eventHandler.emit("setPresence", connectionObj, msg.msgData.setPresence, socketCallback, next);
                    break;

                case "roomJoin":
                    pub.events._eventHandler.emit("roomJoin", connectionObj, msg.msgData.roomJoin, socketCallback, next);
                    break;

                case "roomLeave":
                    pub.events._eventHandler.emit("roomLeave", connectionObj, msg.msgData.roomLeave, socketCallback, next);
                    break;

                case "getRoomList":
                    pub.events._eventHandler.emit("getRoomList", connectionObj, socketCallback, next);
                    break;
                ...
                case "hangup":
                    ...
                    connectionObj.getApp().connection(msg.targetEasyrtcid, function(err,targetConnectionObj){
                        ...
                        pub.events._eventHandler.emit("emitEasyrtcCmd", targetConnectionObj, msg.msgType, outgoingMsg, null, next);
                        socketCallback({msgType:'ack'});
                    });
                    break;
        ...
        function(asyncCallback) {
            ...
            if (msg.targetEasyrtcid !== undefined) {
                ...
                async.waterfall([
                    ...
                    function(asyncCallback) {
                        pub.events._eventHandler.emit("emitEasyrtcMsg", targetConnectionObj, msg.msgType, outgoingMsg, null, asyncCallback);
                    }
                ...
                function (err) {
                    if (err) {
                        ...
                        pub.events._eventHandler.emit("emitReturnError", connectionObj, "UNHANDLED", function(err){});
                    } else {
                        socketCallback({msgType:'ack'});
                    }
                });
                        ...
                        for (var i = 0; i < connectedEasyrtcidArray.length; i++) {
                            ...
                            connectionObj.getApp().connection(connectedEasyrtcidArray[i], function(err, targetConnectionObj){
                                ...
                                if(msg.targetGroup) {
                                    targetConnectionObj.isInGroup(msg.targetGroup, function(err, isAllowed){
                                        if (isAllowed) {
                                            pub.events._eventHandler.emit("emitEasyrtcMsg", targetConnectionObj, msg.msgType, outgoingMsg, null, function(err){});
                                        }
                                    });
                                }
                                else {
...
module.exports.onEmitError      = function(connectionObj, errorCode, socketCallback, next){
    ...
    pub.events._eventHandler.emit("emitEasyrtcCmd", connectionObj, "error", pub.util.getErrorMsg(errorCode), socketCallback, next);
};
...
module.exports.onRoomJoin      = function(connectionObj, rooms, socketCallback, next){
    ...
    if(!_.isObject(rooms) || _.isEmpty(rooms)) {
        pub.events._eventHandler.emit("emitReturnError", socketCallback, "MSG_REJECT_BAD_STRUCTURE", function(err){});
        return;
    }
...
module.exports.onSetPresence      = function(connectionObj, presenceObj, socketCallback, next){
    ...
    pub.util.logDebug("Running func 'onSetPresence' with setPresence: ",presenceObj);
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            pub.util.logDebug("["+connectionObj.getEasyrtcid()+"] EasyRTC info: unhandled ACK return message.", returnMsg);
        };
    ...
    connectionObj.setPresence(
        ...
        function(err){
            if (err) {
                socketCallback(socketCallback(pub.util.getErrorText("MSG_REJECT_PRESENCE")));
            }
            else {
                connectionObj.emitRoomDataDelta(false, function(err, roomDataObj){
                    if (err) {
    ...
    async.waterfall([
        function(callback) {
            ...
            pub.httpApp.configure( function() {
                ...
                if (pub.getOption("apiEnable")) {
                    ...
                    pub.httpApp.get(pub.getOption("apiPublicFolder") + "/easyrtc_ft.js",               function(req, res) {res.sendfile("api/easyrtc_ft.js",                {root:__dirname + "/../"});});
                    pub.httpApp.get(pub.getOption("apiPublicFolder") + "/easyrtc.css",                 function(req, res) {res.sendfile("api/easyrtc.css",                  {root:__dirname + "/../"});});
                    pub.httpApp.get(pub.getOption("apiPublicFolder") + "/easyrtc.min.js",              function(req, res) {res.sendfile("open_source/api/easyrtc.min.js",   {root:__dirname + "/../"});});
                    pub.httpApp.get(pub.getOption("apiPublicFolder") + "/easyrtc_ft.min.js",           function(req, res) {res.sendfile("api/easyrtc_ft.min.js",            {root:__dirname + "/../"});});
                    pub.httpApp.get(pub.getOption("apiPublicFolder") + "/easyrtc.min.css",             function(req, res) {res.sendfile("open_source/api/easyrtc.min.css",  {root:__dirname + "/../"});});
        ...
        function(callback) {
            ...
            pub.socketServer.sockets.on("connection", function (socket) {
                ...
                pub.events._eventHandler.emit("connection", socket, easyrtcid, function(err){
