        ...
        function(callback) {
            ...
            connectionObj.socket.on('easyrtcCmd', function(msg) {
                ...
                eEvents.emit('easyrtcCmd', easyrtcid, msg, function(err){
                    if(err){eu.socketDisconnect(connectionObj.socket);eu.logError("["+easyrtcid+"] Unhandled easyrtcCmd handler error. Disconnecting socket.", err);return;}
                });
            });
            ...
            connectionObj.socket.on('message', function(msg) {
                ...
                eEvents.emit('message', easyrtcid, msg, function(err){
                    if(err){eu.socketDisconnect(connectionObj.socket);eu.logError("["+easyrtcid+"] Unhandled message handler error. Disconnecting socket.", err);return;}
                });
            });
