        ...
        case "hangup":
            if (msg.targetEasyrtcid) {
                ...
                connectionObj.getApp().connection(msg.targetEasyrtcid, function(err,targetConnectionObj){
                    ...
                    pub.eventHandler.emit("emitEasyrtcCmd", targetConnectionObj, msg.msgType, outgoingMsg, next);
                });
            } else {
...
module.exports.onEasyrtcMsg = function(connectionObj, msg, socketCallback, next){
    ...
    if (msg.targetEasyrtcid) {
        ...
        connectionObj.getApp().connection(msg.targetEasyrtcid, function(err,targetConnectionObj){
            if (err){console.log("ERROR", err);return;}
            pub.eventHandler.emit("emitEasyrtcMsg", targetConnectionObj, msg.msgType, outgoingMsg, next);
        });
    } else {
 ...
 * The msgId and serverTime fields will be added to the msg automatically.
 */
module.exports.onEmitEasyrtcMsg = function(connectionObj, msgType, msg, next){
