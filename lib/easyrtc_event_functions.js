...
module.exports.onConnection = function(easyrtcid, next){
    ...
    async.waterfall([
            ...
            connectionObj.socket.on("easyrtcCmd", function(msg, socketCallback) {
                ...
                if(_.isFunction(socketCallback)) {
                    setTimeout(
                        ...
                        function(){pub.util.logWarning("["+easyrtcid+"] Sending return message");socketCallback("Got your message: " + JSON.stringify(msg));},
                        2000
                    )
                    pub.util.logWarning("["+easyrtcid+"] easyRTC command message received with callback:)", msg);
                } else {
                ...
                pub.eventHandler.emit("easyrtcCmd", connectionObj, msg, function(err){
        ...
        function(callback) {
            ...
            var tokenMsg = {
                applicationName: connectionObj.getApp().getAppName(),
                iceConfig: {iceServers: pub.getOption("iceServers")}
            };
            pub.eventHandler.emit("emitEasyrtcCmd", connectionObj, "token", tokenMsg, function(err){
                if(err){pub.util.logError("["+easyrtcid+"] Unable to send token to user.", err);return;}
 ...
 */
module.exports.onEasyrtcCmd = function(connectionObj, msg, next){
