...
module.exports = function Connection(appName, socketId, easyrtcid, appObj, server) {
    ...
    // TODO async .getApp(appName, callback);
     ...
     */
    connectionObj.getApp = function(callback) {

        if (!callback) {
            console.warn('use getApp with callback instead.');
            return appObj;
        }

        return server.getAppName(appName, callback);
    };
     ...
     */
    connectionObj.getSession = function(callback) {
        if (
            server.app[appName].connection[easyrtcid] &&
