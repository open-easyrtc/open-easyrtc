...
var eventHandler   = require("./easyrtc_event_handler");    // EasyRTC default event handlers
...
pub.events.defaultListeners = {
    "authenticate":         eventHandler.onAuthenticate,
    "authenticated":        eventHandler.onAuthenticated,
    "connection" :          eventHandler.onConnection,
    "disconnect":           eventHandler.onDisconnect,
    "roomCreate":           eventHandler.onRoomCreate,
    "roomJoin":             eventHandler.onRoomJoin,
    "roomLeave":            eventHandler.onRoomLeave,
    "log" :                 eventHandler.onLog,
    "shutdown":             eventHandler.onShutdown,
    "startup" :             eventHandler.onStartup,

    "easyrtcAuth" :         eventHandler.onEasyrtcAuth,
    "easyrtcCmd":           eventHandler.onEasyrtcCmd,
    "easyrtcMsg" :          eventHandler.onEasyrtcMsg,

    "emitEasyrtcCmd":       eventHandler.onEmitEasyrtcCmd,
    "emitEasyrtcMsg":       eventHandler.onEmitEasyrtcMsg,
    "emitError":            eventHandler.onEmitError,

    "emitReturnAck":        eventHandler.onEmitReturnAck,
    "emitReturnError":      eventHandler.onEmitReturnError,
    "emitReturnToken":      eventHandler.onEmitReturnToken,

    "msgTypeGetRoomList":   eventHandler.onMsgTypeGetRoomList,
    "msgTypeRoomJoin":      eventHandler.onMsgTypeRoomJoin,
    "msgTypeRoomLeave":     eventHandler.onMsgTypeRoomLeave,
    "msgTypeSetPresence":   eventHandler.onMsgTypeSetPresence
};
...
pub.app = function(appName, callback) {
    ...
    appObj.connection = function(easyrtcid, callback) {
        ...
        connectionObj.getSessionObj = function(callback) {
            // TODO: Remove this method. Use getSession() instead.
        ...
        connectionObj.joinSession = function(sessionKey, next) {
            ...
            appObj.session(sessionKey, function(err, sessionObj){
                if (err) {
                    next(err);
                    return;
                }
        ...
        if (e.app[appName].connection[easyrtcid].toSession) {
            appObj.session(e.app[appName].connection[easyrtcid].toSession.sessionKey, function(err, sessionObj){
                if (err) {
                    callback(err);
                    return;
                }
