...
pub.util.isValidIncomingMessage = function(type, msg, appObj, callback) {
    ...
    var getOption = (_.isObject(appObj) ? appObj.getOption : server.getOption);
...
pub.getAppNames = function(callback) {
    ...
    var appNames = Object.keys(pub.server.app);
 ...
 */
pub.getVersion = server.getVersion.bind(server);
 ...
 */
pub.setOption = server.setOption.bind(server);
 ...
 */
pub.getOption = server.getOption.bind(server);
 ...
 */
pub.isApp = server.isApp.bind(server);
...
pub.createApp = function(appName, options, callback) {
    ...
    if (!appName || !server.getOption("appNameRegExp").test(appName)) {
...
pub.app = function(appName, callback) {
    ...
    if (!appName) {
        appName = server.getOption("appDefaultName");
    }
