...
pub.events.setDefaultListeners = function() {
    ...
    for (var key in pub.events.defaultListeners) {
        pub.eventHandler.on(key, pub.events.defaultListeners[key]);
    }
};
...
pub.util.getErrorMsg = function(errorCode) {
    ...
    if (!msg.msgData.errorText) {
        msg.msgData.errorText="Error occured with error code: " + errorCode;
...
pub.app = function(appName, callback) {
    ...
    appObj.connection = function(easyrtcid, callback) {
        ...
        /**
         * Generates a full room connection clientList for the given connection 
         ...
         */
        connectionObj.getRoomConnectionList = function(roomStatus, roomMap, callback) {
            ...
            for (var currentRoomName in e.app[appName].connection[easyrtcid].room) {
                ...
                for (var currentEasyrtcid in connectionRoom.toRoom.connectList) {
                    ...
                    roomData[currentRoomName].clientList[currentEasyrtcid] = {
                        ...
                        roomJoinTime:   connectionRoom.toRoom.connectList[currentEasyrtcid].toConnection.room[currentRoomName].enteredOn,
                    ...
                    if(!_.isEmpty(connectionRoom.toRoom.connectList[currentEasyrtcid].toConnection.apiField)) {
                        roomData[currentRoomName].clientList[currentEasyrtcid].apiField= connectionRoom.toRoom.connectList[currentEasyrtcid].toConnection.apiField;
                    }
                }
        ...
        connectionObj.room = function(roomName, callback) {
            ...
            connectionRoomObj.emitRoomDataDelta = function(isLeavingRoom, next) {
                ...
                msg.msgData.roomData[roomName] = {roomName:roomName, roomStatus:"update", clientListDelta:{}};

                // TODO: Better protection for when users have disconnected or left a room prior to sending this delta

                if (isLeavingRoom) {
                    msg.msgData.roomData[roomName].clientListDelta.removeConnection = {};
                    msg.msgData.roomData[roomName].clientListDelta.removeConnection[easyrtcid] = {easyrtcid:easyrtcid};
                } else {
                    var connectionRoom = e.app[appName].connection[easyrtcid].room[roomName];
                    msg.msgData.roomData[roomName].clientListDelta.updateConnection = {};
                    // TODO: Add additional fields
                    msg.msgData.roomData[roomName].clientListDelta.updateConnection[easyrtcid] = {
                        easyrtcid:easyrtcid,
    ...
    appObj.group = function(groupName, callback) {
        ...
        groupObj.getConnections = function(callback) {
            ...
            for (var key in e.app[appName].group[groupName].connectList) {
