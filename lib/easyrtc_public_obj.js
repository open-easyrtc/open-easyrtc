...
pub.app = function(appName, callback) {
    ...
    /**
     * Checks if a provided room is defined. This is a synchronous function, thus may not be available in cases where room state is not kept in memory.
     *
     * @memberof    pub.appObj
     * @param       {string} roomName       Room name which uniquely identifies a room within an EasyRTC application.
    ...
    appObj.isRoomSync = function(roomName) {
        return (e.app[appName] && e.app[appName].room[roomName] && !e.app[appName].room[roomName].deleted);
    };
