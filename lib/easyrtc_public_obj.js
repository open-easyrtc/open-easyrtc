...
pub.util.getErrorText = function(errorCode) {
    switch (errorCode) {
        ...
        case "MSG_REJECT_GEN_FAIL":
            return "Message rejected. General failure occured.";
            break;
...
pub.app = function(appName, callback) {
     ...
     * @memberof    pub.appObj
     * @param       {string} roomName       Room name which uniquely identifies a room within an EasyRTC application.
     * @param       {function(Error, count)} callback Callback with error and client count
     * This method was added by Eric.
     */
    appObj.getRoomOccupantCount = function(roomName, callback) {
        ...
        if (e.app[appName].room[roomName]) {
            for (a in e.app[appName].room[roomName].clientList) {
                count++;
            }
        }
        callback(null, count);
     ...
     * @memberof    pub.appObj
     * @param       {string} roomName       Room name which uniquely identifies a room within an EasyRTC application.
     * @param       {?object}   options     Options object with options to apply to the room. May be null. Are ignored.
     * @param       {function(Error, Object)} callback Callback with error and true if a room was deleted.
     * This method was added by Eric.
     */
    appObj.deleteRoom = function(roomName, options, callback) {
        if (!roomName) {
            var msg = "Can't delete room with a null room name";
            pub.util.logWarning(msg);
            callback(new pub.util.ApplicationWarning(msg), false);
            return;
        }

        // if the room has already been delete, don't whine about it.
        if (!e.app[appName].room[roomName]) {
            callback(null, false);
        }

        var count = 0;
        for (var i in e.app[appName].room[roomName].clientList) {
            count++;
        }
        if (count > 0) {
            var msg = "Can't delete room " + roomName + " because it isn't empty";
            pub.util.logWarning(msg);
            callback(new pub.util.ApplicationWarning(msg), false);
            return;
        }
