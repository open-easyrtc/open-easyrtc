...
var EventEmitter = require('./classes/event-emitter');
...
pub.getAppNames = function(callback) {
    ...
    var appNames = Object.keys(easyrtc.app);
...
pub.getAppWithEasyrtcid = function(easyrtcid, callback) {
    for (var appName in easyrtc.app) {
        if (easyrtc.app.hasOwnProperty(appName)) {
            if (
...
pub.getAvailableEasyrtcid = function() {
    ...
    do {
        ...
        for (var key in easyrtc.app) {
            if (easyrtc.app[key].connection[newEasyrtcid]) {
...
pub.getConnectionCountSync = function() {
    ...
    for (var appName in easyrtc.app) {
        if (easyrtc.app.hasOwnProperty(appName)) {
            connectionCount = connectionCount + _.size(easyrtc.app[appName].connection);
        }
    }
...
pub.setOption = function(optionName, optionValue) {
    ...
    if (easyrtc.option.hasOwnProperty(optionName)) {
        easyrtc.option[optionName] = pub.util.deepCopy(optionValue);
        return true;
    } else {
...
pub.isApp = function(appName, callback) {
    callback(null, (easyrtc.app[appName] ? true : false));
};
...
pub.app = function(appName, callback) {
    ...
    const appObj = new Application(appName)
