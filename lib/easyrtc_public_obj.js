 ...
 *
 * @param       {Object} callback   A callback function of form callback(err, appNames) 
 */
 ...
 * @param       {Object} callback Optional callback function of form callback(err, appObj) 
 */
 ...
 * @param       {Object} callback Callback. Optional callback function of form callback(err, appObj) 
 */
p.app = function(appName, callback) {
     ...
     *
     * @param       {Object} callback       A callback function of form callback(err, fieldNames) 
     */
     ...
     *
     * @param       {Object} callback       A callback function of form callback(err, fieldValue) 
     */
     ...
     * @oaram       {Object} next           A success callback of form next(err) 
     */
    ...
    appObj.createSession = function(sessionKey, callback) {
        if (!sessionKey || !p.getOption('sessionKeyRegExp').test(sessionKey)) {
            ...
            next(new p.util.ConnectionWarning("Can not create session with improper name: '" + sessionKey +"'"));
            return;
        }
        if (e.app[appName].session[sessionKey]) {
            p.util.logWarning("Can not create session which already exists: '" + sessionKey +"'");
            next(new p.util.ConnectionWarning("Can not create session which already exists: '" + sessionKey +"'"));
            return;
        }
    ...
    appObj.connection = function(easyrtcid, callback) {
        ...
        * @param       {Object} callback       A callback function of form callback(err, fieldNames)
        ...
        connectionObj.setField = function(fieldName, fieldValue, fieldOptions, next) {
            ...
            if (!p.getOption('fieldNameRegExp').test(fieldName)) {
                p.util.logWarning("Can not create field with improper name: '" + fieldName +"'");
                next(new p.util.ConnectionWarning("Can not create field with improper name: '" + fieldName +"'"));
                return;
            }
        ...
        connectionObj.room = function(roomName, callback) {
            ...
            /* Sets room field value for a given field name. Returns false if field could not be set.
             *  
             * @param       {String} fieldName      Must be formatted according to 'fieldNameRegExp' option.
             * @param       {Object} fieldValue
             * @param       {Object} fieldOptions   Field options (to be implemented in future. Options for sharing fields to the API with possible group restrictions)
            ...
            roomObj.setField = function(fieldName, fieldValue, fieldOptions, next) {
                if (!p.getOption('fieldNameRegExp').test(fieldName)) {
                    p.util.logWarning("Can not create field with improper name: '" + fieldName +"'");
                    callback(new p.util.ConnectionWarning("Can not create field with improper name: '" + fieldName +"'"));
                    return;
                }
