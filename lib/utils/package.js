...
package_utils.getPackageData = function(packagePath, fieldName) {
    const packageData = package_utils.getPackage(packagePath);

    if (!fieldName) {
        return utils_easyrtc.deepCopy(packageData);
    }
    else if (packageData[fieldName]) {
        return utils_easyrtc.deepCopy(packageData[fieldName]);
    }
    else {
        return null;
    }
...
package_utils.checkModules = function (packagePath) {
    ...
    for (let moduleName in packageData.dependencies) {
        if (packageData.dependencies.hasOwnProperty(moduleName)) {
